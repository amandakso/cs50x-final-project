...
def index(): 
    ...
    if request.method == "POST":
        if request.form.get("cal") == "2":
            ...
            events = db.execute("SELECT title, date, starttime, endtime, details FROM events WHERE creator = ? and date BETWEEN ? AND ? ORDER BY date, starttime", session.get("user_id"), date1, date2)
            num_of_events = []
            for day in days:
